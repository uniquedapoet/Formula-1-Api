...
from app.models.pydantic_model import Circuit as PydanticCircuit  # Import the Pydantic model
...
def get_circuit(circuit_id: int, db: Session = Depends(get_db)):
    circuit = db.query(DBCircuit).filter(DBCircuit.id == circuit_id).first()  # Use the SQLAlchemy model here
    if not circuit:
